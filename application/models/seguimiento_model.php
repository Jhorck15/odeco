...
var $lista = array();
	...
	public function getReclamo(){
		...
		$this->db->select('r.id_reclamo, p.nombres, p.apellidos, r.numero, (date(r.fechareclamo)) as ini, (date(r.fecharespuesta)) as fin, f.nombreforma, (zo.numero*100000000 + ma.numeromanzano*10000 + z.vivienda) as codigousuario');
		$this->db->from('funcionario fu');
		$this->db->join('reclamo r', 'r.id_funcionario = fu.id_funcionario');
		$this->db->join('tiporeclamo t', 'r.id_tiporeclamo = r.id_tiporeclamo');
		...
		$this->db->where('t.id_tiporeclamo', 1);
