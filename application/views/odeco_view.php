        ...
        <div class="panel-body ">
            ...
            <div class="row">
                <div class="col-md-4">
                	<div class="form-group">
						<div class="col-md-6">
		                    <button type="button" id="btnbuscarabonado" class="btn btn-success btn-labeled btn-xlg" name="nombre">
	    	...
	    	<div class="row">
	    		<div class="col-md-4">
	    			<div class="form-group">
	                	...
	                	<input type="hidden" id="cod_usu_odeco" name="cod_usu_odeco" readonly="true">
					</div>
				</div>
				<div class="col-md-4">
	    			<div class="form-group">
						...
						<div class="row">
							<div class="col-md-12">
								<div class="content-group">
									<label>Motivo: </label>
									<div class="form-group">
										<textarea rows="4" cols="1" id="motivomodal" name="motivomodal" class="form-control text-uppercase elastic border-success border-lg" placeholder="Motivos por el que reclama"></textarea>
									</div>
								</div>
							</div>
...
<script type="text/javascript">
		...
		$("body").on("submit", "#form_persona_nuevo", function(e){
            ...
            $.ajax({
				...
				success: function(d) {
					...
					if(d.status){
						if (d.status == '100'){
							alert('Ya fue registrado el abonado');
						} else {
     ...
     //                	$('#nombrereclamante').val(d.persona.nombres+' '+d.persona.apellidos);	
		...
		table = $('#tabla').DataTable({
	        ...
	        "ajax": {
	            ...
	            "url": "<?php echo site_url('Odeco/ajax_listvista')?>",
	            "type": "POST"
	        },
					        ...
					        '<form id="form_persona_actualizar">'+
					            ...
					            '<div class="row">'+
					                '<div class="col-md-4">'+
					                    '<div class="form-group">'+
					                        '<label>NIT: </label>'+
					                        '<input id="nit" name="nit" value="'+persona.nits+'" class="form-control bg-success" type="text">'+
		...
		function makeAjaxRequest() {
			$('#cod_usu_odeco').val($('input#nombre').val());
		    var parametros = {
	...
	function update_reclamomodal()
	{
	    ...
	    $.ajax({
	        url : "<?php echo site_url('odeco/ajax_update_reclamo')?>",
	        type: "POST",
	...
	$("#form_reclamo").on("submit", function(event) {
		...
		$.ajax({
			...
			success: function(d) {
				if (d.status) {
					if (d.status == '100'){
						alert('Ya existe un reclamo en curso');
					} else {
						alert('Los datos fueron guardados correctamente');
						numero();
						rep_reclamo();
	...
	function rep_reclamo(){
		...
		// console.log(cod_reclamo);
 		window.open("<?php echo site_url('rep_reclamo/reporte/')?>" + cod_reclamo, "Reporte reclamo", "directories=no, location=no, menubar=no, scrollbars=yes, statusbar=no, tittlebar=yes, width=acnhura, height=altura, top=y,left=x");
	};
	...
	$("#form_reclamomodal").on("submit", function(event) {
		...
		$.ajax({
			...
			success: function(d) {
					...
					// alert('Ya esta registrado el codigo de Usuario...');
