<div class="panel panel-flat">
						<div class="panel-heading">
							<h5 class="panel-title">Lista de Funcionarios</h5>
							<button class="btn btn-success" id="btnFuncionario"><i class="glyphicon glyphicon-plus"></i> Añadir Funcionario</button>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
	  	 ...
	  	 table = $('#t_reclamo').DataTable({ 
			...
			"language": {
	            "info" : "Mostrar pagina _START_ de _END_ de _TOTAL_ funcionarios",
	            "loadingRecords":  "No hay funcionarios...",
	     		"processing" :     "Procesando...",
	            "sSearch" :      "Buscar ",
            	"ZeroRecords":  "No se encontraron resultados",
	            "infoEmpty":  "No hay funcionarios",
	            "lengthMenu":  "Mostrar _MENU_ registros",
       		},	
			
			"ajax": {
		            url: "<?php echo site_url('clista_func/lista')?>",	
		            type: "POST",
	...
	//         // botones 
        	...
        	$('#listage').change(function(){
        		var id = $('#listage').val();
        		// alert(id);
     		    listaje(id);
        	});

        	$('#listaje').change(function(){
        		var id = $('#listaje').val();
        		// alert(id);
     		    listaca(id);
        	});
		...
		function odeco()
		{
		    ...
		    $('.form-group').removeClass('has-error'); // clear error class
		    $('.help-block').empty(); // clear error string
		    $('#modal_imprimir').modal({backdrop: 'static', keyboard: false})
		...
		function edit_funcionario(id)
			{	
			    ...
			    $('.help-block').empty(); 
			     ...
			     $.ajax({
			         ...
			         url: "<?php echo site_url('clista_func/ajax_editt')?>/" + id,
			         type:  "GET",
			         dataType : "JSON",
			         async:false,
			         ...
			         success: function(data)
			         {			         	
			         	listaje(data.id_gerencia);
			         	listaca(data.id_jefatura);
			         	$('[name=nfunc]').val(data.nombres);
			         	$('[name=afunc]').val(data.apellidos);
			         	$('[name=difunc]').val(data.direccion);
			            $('[name=telefunc]').val(data.telefono);
			            $('[name=celfunc]').val(data.celular);
			            $('[name=cifunc]').val(data.telefono);
			            $('[name=nitfunc]').val(data.celular);
			            $('[name=listage]').val(data.id_gerencia);
			            // $("#listaje").val(2);
			            $('[name=listaje]').val(data.id_jefatura);
			            // $('#listaje option[value="2"]').attr('selected', 'selected');
			            $('[name=listaca]').val(data.id_cargo);
			            $('[name=listaro]').val(data.id_rol);
			            $('[name=ctafunc]').val(data.usuario);
			            $('[name=password]').val(data.contrasenia);

			            $('#modal_func').modal('show'); 
			            $('.modal-title').text('Datos del funcionario'); 
			...
			function a_funcionario()
			{
			    ...
			    $('#listaca').empty();
			    $('#modal_func').modal('show'); 
			    $('#modal_func').modal({backdrop: 'static', keyboard: false})  
			    $('.modal-title').text('Añadir datos del funcionario');
			}
			...
			function funcionario()
			{
			    ...
			    $('.help-block').empty(); 
			     ...
			     $.ajax({
			         ...
			         url: "<?php echo site_url('cprincipal/datos_user')?>/",
			         type:  "GET",
			         dataType : "JSON",
			         ...
			         success: function(data)
			         {
			         	...
			         	// text(fr[0]['nombres']
			         	// $('#nomuser').text(fr[0]['nombres']+' '+fr[0]['apellidos']);
			         	$('[name=nombrefunc]').val(data.nombres);
			         	$('[name=apellidofunc]').val(data.apellidos);
			         	  // $('[name="motivo"]').val(data.motivo);			        
			            $('[name=direccfunc]').html(data.direccion);
			            $('[name=telefonofunc]').html(data.telefono);
			            $('[name=celularfunc]').html(data.celular);
			            $('#modal_funcionario').modal('show'); 
			            $('#modal_funcionario').modal({backdrop: 'static', keyboard: false})
			            $('.modal-title').text('Datos personales'); 
			...
			{	
				...
				if ($('#nfunc').val() != ' ') {
				    ...
				    url = "<?php echo site_url('clista_func/ajax_adicionar')?>/";
		...
		$.get('<?php echo base_url('clista_func/listage') ?>', function(data) {
			// alert(data);
			var fr = $.parseJSON(data);
			$('#listage').append('<option value="">Seleccionar...</option>');
			$.each(fr, function(index, val) {
				$('#listage').append('<option value="'+val.id_gerencia+'">'+val.detallegerencia+'</option>');
			});
		});

		$.get('<?php echo base_url('clista_func/listaro') ?>', function(data) {
			// alert(data);
			var fr = $.parseJSON(data);
			$('#listaro').append('<option value="">Seleccionar...</option>');
			$.each(fr, function(index, val) {
				$('#listaro').append('<option value="'+val.id_rol+'">'+val.nombrerol+'</option>');
			});
		});
		...
		function listaje(id){
			// alert(id);
			$('#listaje').empty();
			$.get('<?php echo base_url('clista_func/listaje/') ?>' + id, function(data) {
				// alert(data);
				var fr = $.parseJSON(data);
				$('#listaje').append('<option value="">Seleccionar...</option>');
				$.each(fr, function(index, val) {
					$('#listaje').append('<option value="'+val.id_jefatura+'">'+val.detallejefatura+'</option>');
				});
			});
		}
		...
		function listaca(id){
			// alert(id);
			$('#listaca').empty();
			$.get('<?php echo base_url('clista_func/listaca/') ?>' + id, function(data) {
				// alert(data);
				var fr = $.parseJSON(data);
				$('#listaca').append('<option value="">Seleccionar...</option>');
				$.each(fr, function(index, val) {
					$('#listaca').append('<option value="'+val.id_cargo+'">'+val.detallecargo+'</option>');
				});
			});
		}
...
<div class="modal fade" id="modal_func" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header bg-success">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
				    ...
				    <div class="panel-body">
				    	<div class="row">
				    		<div class="col-md-6">
				    			<div class="form-group">
									<label>Nombres  </label>
				                	<input type="text" class="form-control" name="nfunc" id="nfunc" cols="4" style="text-transform:uppercase;">
								</div>
							</div>
							<div class="col-md-6">
				    			<div class="form-group">
									<label>Apellidos  </label>
				                	<input type="text" class="form-control " name="afunc" id="afunc" style="text-transform:uppercase;">
								</div>
							</div>
						...
						<div class="row">
				    		<div class="col-md-6">
				    			<div class="form-group">
				                	...
				                	<input type="text" class="form-control" name="telefunc" id="telefunc" placeholder="64 46526" cols="4" >
								</div>
							</div>
							<div class="col-md-6">
				    			<div class="form-group">
									<label>Celular  </label>
				                	<input type="text" class="form-control " name="celfunc" id="celfunc" placeholder="72854321" >
								</div>
							</div>
						...
						<div class="row">
				    		<div class="col-md-12">
				    			<div class="form-group">
				                	...
				                	<input type="text" class="form-control" name="difunc" id="difunc" placeholder="Sin dirección" cols="4" style="text-transform:uppercase;">
								</div>
							</div>
						...
						<div class="row">
				    		<div class="col-md-6">
				    			<div class="form-group">
				                	...
				                	<input type="text" class="form-control" name="cifunc" id="cifunc" placeholder="5020577" cols="4" >
								</div>
							</div>
							<div class="col-md-6">
				    			<div class="form-group">
									<label>NIT  </label>
				                	<input type="text" class="form-control " name="nitfunc" id="nitfunc" placeholder="5020577019" >
								</div>
							</div>
						...
						<div class="row">
				    		<div class="col-md-6">
				    			<div class="form-group">
									...
									<div>
					                	<select name="listage" id="listage" class="select2-selection select2-selection--single bg-success">
							...
							<div class="col-md-6">
				    			<div class="form-group">
									...
									<div>
					                	<select name="listaje" id="listaje" class="select2-selection select2-selection--single bg-success">
						...
						<div class="row">
				    		<div class="col-md-6">
				    			<div class="form-group">
									...
									<div>
					                	...
					                	<select name="listaca" id="listaca" class="select2-selection select2-selection--single bg-success">
							...
							<div class="col-md-6">
				    			<div class="form-group">
									...
									<div>
										<span class="selection">
					                	<select name="listaro" id="listaro" class="select2-selection select2-selection--single bg-success">
						...
						<div class="row">
				    		<div class="col-md-6">
				    			<div class="form-group">
									...
									<div>
					                	<input type="text" class="form-control " name="ctafunc" id="ctafunc" placeholder="AFLORES" style="text-transform:uppercase;">
					                </div>
								</div>
							</div>
						...
						<div class="row">
				    		<div class="col-md-6">
				    			<div class="form-group">
										<label class="control-label col-lg-6">Contraseña <span class="text-danger">*</span></label>
										
											<input type="password" name="password" id="password" class="form-control" required="required" placeholder="Mínimo 5 caracteres">
									
									</div>
							</div>
							<div class="col-md-6">
				    			<div class="form-group">
										<label class="control-label col-lg-9">Repetir contraseña <span class="text-danger">*</span></label>
										
											<input type="password" name="repeat_password" class="form-control" required="required" placeholder="Repita la misma contraseña">
...
<div class="sweet-alert showSweetAlert visible" data-custom-class="" data-has-cancel-button="true" data-has-confirm-button="true" data-allow-outside-click="false" data-has-done-function="false" data-animation="pop" data-timer="null" style="display:  block; margin-top:  -5px;"><div class="sa-icon sa-error" style="display:  none;">
    ...
    <p style="display:  block;">De guardar datos del funcionario!</p>
    ...
    <fieldset>
      ...
      <div class="sa-confirm-button-container">
        <button class="confirm" tabindex="1" style="display:  inline-block; background-color:  rgb(255, 112, 67); box-shadow:  rgba(255, 112, 67, 0.8) 0px 0px 2px, rgba(0, 0, 0, 0.05) 0px 0px 0px 1px inset;" onclick="guardar()">Si, guardar datos!</button><div class="la-ball-fall">
          <div></div>
          <div></div>
          <div></div>
        </div>
