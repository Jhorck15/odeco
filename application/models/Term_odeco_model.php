<?php
Class Term_odeco_model extends CI_Model {
var $table = 'reclamo r';
var $usuarioid = '';
...
var $lista = array();
	...
	public function getReclamo(){
		...
		$this->db->select('r.id_reclamo, p.nombres, p.apellidos, r.numero, (date(r.fechareclamo)) as ini, (date(r.fecharespuesta)) as fin, t.nombretiporeclamo, (zo.numero*100000000 + ma.numeromanzano*10000 + z.vivienda) as codigousuario, r.id_tiporeclamo');
		...
		$this->db->where('r.estadoreclamo', '5');
	...
	private function _get_datatables_query()
	{
			...
			$this->db->select('r.id_reclamo, p.nombres, p.apellidos, r.numero, (date(r.fechareclamo)) as ini, (date(r.fecharespuesta)) as fin, t.nombretiporeclamo, (zo.numero*100000000 + ma.numeromanzano*10000 + z.vivienda) as codigousuario');
			$this->db->from('funcionario fu');
			$this->db->join('reclamo r', 'r.id_funcionario = fu.id_funcionario');
			$this->db->join('persona p', 'p.id_persona = r.id_persona');
			$this->db->join('formareclamo f', 'f.id_formareclamo = r.id_formareclamo');
			...
			$this->db->join('zona zo', 'zo.id_zona = z.id_zona');
			$this->db->join('manzano ma', 'ma.id_manzano = z.id_manzano');		
			$this->db->where('r.estadoreclamo', '6');
			$this->db->order_by('r.numero','desc');
