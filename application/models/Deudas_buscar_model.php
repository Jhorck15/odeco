<?php
Class Deudas_buscar_model extends CI_Model {
var $table = 'reclamo';
var $usuarioid = '';
var $datoins = 'INSPECTOR';
var $column = array('concepto', 'lectura_anterior', 'lectura_actual', 'consumo','monto_total'); 
var $lista = array();
	...
	public function getReclamo($dato){
	...
	public function count_all($dato)
	{
...
// order by r.numero desc
 ...
 //    	$this->db->where('codigousuario', $dato);
    ...
    public function buscar_odeco($dato){
    	$this->db->from('reclamo');
    	$this->db->where('codigousuario', $dato);
    	$this->db->where('estadoreclamo', '6');
    	$cons = $this->db->get();
    	return $con->row();
    }
    ...
    public function get_datoaguas(){
		// $this->db->select('f.id_funcionario, p.nombres, p.apellidos, c.detallecargo');
		$this->db->select('f.id_funcionario, p.nombres, p.apellidos, c.detallecargo');
		$this->db->from('persona p');
		$this->db->join('funcionario f','f.id_persona = p.id_persona');
		$this->db->join('cargo c','c.id_cargo = f.id_cargo');
		$this->db->where('c.detallecargo','INSPECTOR RED DE AGUA');
	...
	public function get_datoalc(){
		// $this->db->select('f.id_funcionario, p.nombres, p.apellidos, c.detallecargo');
		$this->db->select('f.id_funcionario, p.nombres, p.apellidos, c.detallecargo');
		$this->db->from('persona p');
		$this->db->join('funcionario f','f.id_persona = p.id_persona');
		$this->db->join('cargo c','c.id_cargo = f.id_cargo');
		$this->db->where('c.detallecargo','INSPECTOR ALCANTARILLADO');
