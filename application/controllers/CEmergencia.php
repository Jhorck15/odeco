...
class CEmergencia extends CI_Controller {
	...
	public function ajax_listvista()
	{
		...
		$output = array(
			"draw" => $_POST['draw'],
			"recordsTotal" => $this->emergencia_model->count_all(),
			"recordsFiltered" => $this->emergencia_model->count_filtered(),
			"data" => $data,
		);
	...
	{
		$list = $this->emergencia_model->get_datatables();
		$data = array();
		$no = $_POST['start'];
		foreach ($list as $reclamo) {
			$no++;
	...
	{
		$clasereclamo = $this->emergencia_model->get_clasereclamo();
		echo json_encode($clasereclamo);
	}
	public function get_formareclamo()
	{
    	$formareclamo = $this->emergencia_model->get_formareclamo();
    	echo json_encode($formareclamo);
	}

	public function get_numeroreclamo()
	{
    	$numero = $this->emergencia_model->get_numeroreclamo();
	...
	public function get_abonado_by_cuentausuario()
	{
		...
		$list = $this->emergencia_model->get_abonado_by_cuenta_usuario($co);
	...
	public function ajax_add_reclamo(){
		...
		// if ( !empty($_POST["tiporeclamo"]) && is_array($_POST["tiporeclamo"]) ) { 
			...
			// print_r($data);
			$this->emergencia_model->save_reclamo($data);
	...
	public function get_tiporeclamo_subreclamo(){
		$list = $this->emergencia_model->get_tiporeclamo_subreclamo();
		// print_r($list);
		echo json_encode($list);
	}

	public function get_tiporeclamo(){
		$list = $this->emergencia_model->get_tiporeclamo();
		echo json_encode($list);
	}

	public function get_subreclamo(){
		$id_tiporeclamo = $this->input->post('id_tiporeclamo');
		$list = $this->emergencia_model->get_subreclamo($id_tiporeclamo);
		echo json_encode($list);
	}

	public function ajax_edit_reclamo($id)
	{
		$data = $this->emergencia_model->get_by_id($id);
	...
	public function ajax_update_reclamo(){
		if ( !empty($_POST["formareclamomodal"]) || !empty($_POST["tiporeclamomodal"]) || !empty($_POST["clasereclamomodal"]) ) { 
			...
			$this->emergencia_model->update(array('id_reclamo' => $this->input->post('id_reclamomodal')), $data);
			echo json_encode(array("status" => TRUE));
		} else {
			echo json_encode(array("status" => FALSE));
		}
