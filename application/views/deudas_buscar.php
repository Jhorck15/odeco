<div class="panel panel-flat bg-danger-300">
	<div class="panel-heading">
		<h5 class="panel-title">Pagos pendientes</h5><p> </p>
		<!-- <a href="<?php echo base_url('odeco') ?>"><button class="btn btn-success"><i class="glyphicon glyphicon-plus"></i> Añadir Reclamo</button></a> -->
    	...
    	<div class="row">
    		<div class="col-md-6">
    			<div class="form-group">
                	...
                	<input autofocus type="text" class="form-control" name="codusuario" id="codusuario" placeholder="Ingresar Código de Usuario" cols="4" >				                	
                </div>				               
			</div>
			<div class="col-md-6">
					<div  class="form-group"><div> </div> <div><p></p> </div><div class="color:#E57373;"> jhorck</div> <div> </div>
                	<button type="button" id="btnbuscarpagos" class="btn bg-teal-4  ">Buscar <i class=" icon-search4" ></i></button>
                	</div>
            </div>							
		</div>
	</div>
...
<div class="panel panel-flat">
	<table class="table datatable-basic table-bordered table-striped table-hover" id="t_deudas">
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		// var combo = documento.getElementById('idlistaaguas');
		$("#listainsp").change(function(){
				$('#listaaguas').empty();
		        var combo = $('#listainsp').val();
		        if(combo == '191'){					
					$.get('<?php echo base_url('cprincipal/listaagua') ?>', function(data) {
						// alert(data);
					var fr = $.parseJSON(data);
					
						$.each(fr, function(index, val) {
								
								$('#listaaguas').append('<option value="'+fr[index]['id_funcionario']+'">'+fr[index]['detallecargo']+' '+fr[index]['nombres']+' '+fr[index]['apellidos']+'</option>');
						});
					});
				}
				...
				if(combo == '192'){	
					// alert(combo);
					$('#listaaguas').empty();				
					$.get('<?php echo base_url('cprincipal/listaalc') ?>', function(data) {
			...
			$('#btnbuscarpagos').click(function(){
				...
				if(campo != '') {
					$('#btnbuscarpagos').html('<div id="cargando"><img style="with:50px; height:20px;" src="<?php echo base_url()?>assets/images/ela/loading.gif"></div>');
					 // $('#btnbuscarabonado').hide();
					 $('#btnbuscarpagos').attr('disabled', true);
				    // makeAjaxRequest();
				    buscar_deuda();
					
				} else {	
					$('#resultado').html('<div class="alert bg-info alert-styled-right">										<button type="button" class="close" data-dismiss="alert"><span>×</span><span class="sr-only">Close</span></button><span class="text-semibold">Vacio!</span> Debe ingresar el codigo de usuario... <a href="#" class="alert-link">Super importante</a>.</div>');
   ...
   //  					alert('dasdasd');
			...
			// 	} else {	
			// 		$('[name="abonadores"]').html('No tiene reclamos el usuario...');
	    ...
	    var parametros = {
	  	 ...
	  	 table = $('#t_deudas').DataTable({ 
			...
			"language": {
	            ...
	            "loadingRecords":  "No hay pendientes...",
	     		"processing" :     "Procesando...",
	            "sSearch" :      "Buscar ",
            	"ZeroRecords":  "No se encontraron deudas",
	            "infoEmpty":  "No hay pendientes",
	            "lengthMenu":  "Mostrar _MENU_ registros",
           
       		},	
			// $fechaactual = date("d-m-Y",strtotime('now'));
			"ajax": {
		            url: "<?php echo site_url('cdeudas_buscar/lista')?>",	
		            type: "POST",
		            data: parametros,

		         //    success:  function(response) {
		         //    	// alert(response);
			        // 	$('#btnbuscarpagos').append('<div style="color:black;">Buscar </div>')
			        //  	$('#btnbuscarpagos').text('Buscar ');
			        //  	// $('#abonadores').empty();
			        // 	$('#btnbuscarpagos').attr('disabled', false);			           
			        // },
		 	...
		 	function makeAjaxRequest() {
			    ...
			    $.ajax({
			        ...
			        success:  function(data) {
			            ...
			            // $('#abonado').val(data.nombres);
			            // $('#apellidoabonado').val(data.apellidos);
			            // $('#cargando').remove();
			            // $('#btnbuscarabonado').html('Buscar');
	...
	// function get_jsonp() {
 //    $.ajax({
 //        url  'http //localhost/occ4/inspeccion/',//esto con un archivo php
   	...
   	// 				$('input:text:visible:first').focus();
			...
			function guardar()
			{
			    ...
			    if(id2 == null){
				   ...
				   //          funcnumero();
				    ...
				    if(save_method == 'add') {
				        url = "<?php echo site_url('cprincipal/ajax_adicionar')?>/" + id2;
				    } 
				    // else {
				    //     url = "<?php echo site_url('cprincipal/ajax_update')?>";
				    // }
			...
			$.get('<?php echo base_url('cprincipal/listainspectorr') ?>', function(data) {
				...
				$.each(fr, function(index, val) {
					...
					// else if (val.detallecargo == 'JEFE ATENCION AL CLIENTE'){
					// 	$('#listainsp').append('<option value="'+val.id_funcionario+'">'+val.detallecargo+'</option>');
					// } else if (val.detallecargo == 'JEFE ALCANTARILLADO'){
					// 	$('#listainsp').append('<option value="'+val.id_funcionario+'">'+val.detallecargo+'</option>');
					// }else if (val.detallecargo == 'JEFE MEDICION Y FACTURACION'){
					// 	$('#listainsp').append('<option value="'+val.id_funcionario+'">'+val.detallecargo+'</option>');
					// }else if (val.detallecargo == 'JEFE RED DE AGUA'){
					// 	$('#listainsp').append('<option value="'+val.id_funcionario+'">'+val.detallecargo+'</option>');
					// }
					else {
                ...
                <form id="form_inspector">
				    ...
				    <div class="panel-body">
						...
						<div class="row">
							...
							<div class="text-center">
								<select name="listaaguas" id="listaaguas">
					               <?php  
					               // print_r($listaaguas); 	                       
					                  foreach($listaaguas as $cargar){
					                   echo '<option name="listaaguas" value="'.$cargar->id_funcionario.'">'.$cargar->nombres.' '.$cargar->apellidos.'</option>';
					                }
				                	?>
				                </select>
							</div>
