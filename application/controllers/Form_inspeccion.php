...
class Form_inspeccion extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model('Form_inspeccion_model');
	}

	public function index()
	{
	    $data['vista'] = 'form_inspeccion_view';
	    $this->salida($data);
	}
	...
	public function ajax_list()
	{
		...
		foreach ($list as $inspeccion) {
			...
			$row[] = '<ul class="icons-list">
						<li class="text-primary-600">
							<a href="javascript:void()" title="Edit" onclick="edit_forminscripcion('."'".$inspeccion->id_formularioinspeccion."'".')"><i class="icon-pencil7"></i></a>
						</li>
		...
		$output = array(
			"draw" => $_POST['draw'],
			"recordsTotal" => $this->Form_inspeccion_model->count_all(),
			"recordsFiltered" => $this->Form_inspeccion_model->count_filtered(),
			"data" => $data,
		);
	...
	{
		// print_r($id);
		$data = $this->Form_inspeccion_model->get_by_id($id);
		$data->fechaforminspeccion = ($data->fechaforminspeccion == '0000-00-00') ? '' : $data->fechaforminspeccion;

		$data2 = $this->Form_inspeccion_model->get_by_idformularioinspeccion($data->id_formularioinspeccion);
	...
	public function get_subreclamo(){
		...
		$list = $this->Form_inspeccion_model->get_subreclamo($id_tiporeclamo);
	...
	{
		$archivo = $this->Form_inspeccion_model->get_archivo($this->input->post('id_archivo'));
