...
class Form_ins_tecnica_model extends CI_Model {

	var $table = 'forinscuatro';
	var $column = array('fechaforinspcuatro', 'numerohabitantes', 'presionaguazona', 'faltaagua', 'faltapresion', 'fugaredmatriz', 'llavemal', 'malaagua','id_reclamo', 'fugaacometida', 'personaqueatendio'); //set column field database for order and search
	var $order = array('id_forinscuatro' => 'desc'); // default order 
    ...
    public function save_tipreclamo_subreclamo($data){
    	$this->db->insert('tiporeclamocuatro_subreclamo', $data);
    }

    public function get_by_id($id)
	{
		$this->db->from($this->table);
		$this->db->where('id_forinscuatro',$id);
		$query = $this->db->get();
    ...
    {
		$this->db->from('tiporeclamocuatro_subreclamo');
		$this->db->where('id_forinscuatro',$id);
		$query = $this->db->get();
		return $query->result();
    }
    ...
    {
        $this->db->select('nombresubreclamo, id_subreclamo');
	...
	{
		$this->db->where('id_forinscuatro', $id);
		$this->db->delete('tiporeclamocuatro_subreclamo');
	}
	...
	{
		$this->db->insert('formularioinscuatro_archivo', $data);
	}
 ...
 //    }
	...
	public function get_archivo($id)
	{
		...
		$this->db->join('formularioinscuatro_archivo fa','ar.id_archivo = fa.id_archivo');
		$this->db->where('fa.id_forinscuatro',$id);
		$query = $this->db->get();
	...
	{
		$this->db->where('id_forinscuatro', $id);
		$this->db->delete('formularioinscuatro_archivo');
	}
