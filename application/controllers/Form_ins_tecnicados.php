...
class Form_ins_tecnicados extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model('Form_ins_tecnicados_model');
	}

	public function index()
	{
	    $data['vista'] = 'form_ins_tecnicados_view';
	    $this->salida($data);
	}
	...
	{
		$list = $this->Form_ins_tecnicados_model->get_datatables();
		// print_r($list);
		$data = array();
		$no = $_POST['start'];
		foreach ($list as $inspeccion) {
			$no++;
			$row = array();
			$row[] = $inspeccion->fechaforinspcuatrodos;
			$row[] = $inspeccion->obsacometida;
			$row[] = $inspeccion->septicos;
			$row[] = $inspeccion->servidumbres;
			// $row[] = $inspeccion->faltapresion;
			// $row[] = $inspeccion->fugaredmatriz;
			// $row[] = $inspeccion->llavemal;
			// $row[] = $inspeccion->malaagua;
			// $row[] = $inspeccion->fugaacometida;			
			$row[] = '<ul class="icons-list">
						<li class="text-primary-600">
							<a href="javascript:void()" title="Edit" onclick="edit_forminscripcion('."'".$inspeccion->id_forinscuatrodos."'".')"><i class="icon-pencil7"></i></a>
						</li>
		...
		$output = array(
			"draw" => $_POST['draw'],
			"recordsTotal" => $this->Form_ins_tecnicados_model->count_all(),
			"recordsFiltered" => $this->Form_ins_tecnicados_model->count_filtered(),
			"data" => $data,
		);
	...
	public function cargar_reclamo(){
		// $_SESSION['id_reclamosession'] = 140;
	...
	public function ajax_add_inspection(){
		if ( !empty($_POST["subreclamo"]) && is_array($_POST["subreclamo"]) ) {
			if(!empty($_FILES['photo']['name']))
			{
				...
				$data = array(
					'fechaforinspcuatrodos' => $this->input->post('fechaforinspcuatro'),
					...
					'fugaalcanta' => $this->input->post('fugaalcanta'),
					'obsacometida' => $this->input->post('obsacometida'),
					'septicos' => $this->input->post('septicos'),
					'servindumbres' => $this->input->post('servidumbres'),
				...
				// print_r($data);
				$id_forinscuatro = $this->Form_ins_tecnicados_model->save_formularioinspeccion($data);

				foreach ($upload as $up => $value) {
					foreach ($value as $key => $val) {
						if ($key=='file_name') {
							$dataarchivo = array('nombrearchivo' => $val, 'titulo' => '' );
							$id_archivo = $this->Form_ins_tecnicados_model->save_archivo($dataarchivo);
							$datos_form_arch = array(
								'id_forinscuatrodos' => $id_forinscuatro,
								'id_archivo' => $id_archivo
							);
							$this->Form_ins_tecnicados_model->save_formularioinspeccion_archivo($datos_form_arch);
						}
					}
				}
				...
				foreach ($_POST["subreclamo"] as $sr ) {
					...
					$this->Form_ins_tecnicados_model->save_tipreclamo_subreclamo($datos);
				}
